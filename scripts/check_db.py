...
def handle_warn(name, actual_value):
    ...
    if expected_values[name] > actual_value:
        msg = (
            f"WARNING: {name} value is {actual_value}, expected {expected_values[name]}"
        )
        print(msg)
        _logger.info(msg)
...
with psycopg.connect("dbname=dgidb user=postgres") as conn:
    with conn.cursor() as cur:
        ...
        check_value(
            cur, "gene_claim_attributes", "SELECT COUNT(*) FROM gene_claim_attributes;"
        )
        check_value(
            cur, "gene_claim_aliases", "SELECT COUNT(*) FROM gene_claim_aliases;"
        )
        check_value(cur, "drug_claims", "SELECT COUNT(*) FROM drug_claims;")
        check_value(
            cur, "drug_claim_attributes", "SELECT COUNT(*) FROM drug_claim_attributes;"
        )
        check_value(
            cur, "drug_claim_aliases", "SELECT COUNT(*) FROM drug_claim_aliases;"
        )
        check_value(
            cur, "interaction_claims", "SELECT COUNT(*) FROM interaction_claims;"
        )
        check_value(
            cur,
            "interaction_claim_attributes",
            "SELECT COUNT(*) FROM interaction_claim_attributes;",
        )
        check_value(
            cur,
            "interaction_claim_types_interaction_claims",
            "SELECT COUNT(*) FROM interaction_claim_types_interaction_claims;",
        )
        check_value(
            cur,
            "interaction_claims_publications",
            "SELECT COUNT(*) FROM interaction_claims_publications;",
        )
        ...
        check_value(
            cur,
            "gene_category_claims",
            "SELECT COUNT(*) FROM gene_claim_categories_gene_claims;",
        )
        check_value(
            cur, "gene_categorizations", "SELECT COUNT(*) FROM gene_categories_genes;"
        )

        check_value(
            cur, "drug_approval_ratings", "SELECT COUNT(*) FROM drug_approval_ratings;"
        )
